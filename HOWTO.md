...
```bash
brew install mneves75/tap/hay      # macOS and Linux; prebuilt, checksummed, attested
cargo install --locked --path hay  # from this checkout instead; lands in ~/.cargo/bin/hay
```

The Homebrew formula installs the exact archives the release workflow built and attested — it is
generated by `brew-formula.sh`, which refuses to emit a checksum for an artifact whose build
provenance does not verify. It is the supported install path; the source build is for hacking on
`hay` itself.
