...
- `brew-formula.sh` fetches each archive from its public download URL — the exact URL the formula
  records — instead of through `gh release download`. After v0.3.2 was published, GitHub's by-tag
  release endpoint listed no assets for over half an hour while every other view served all ten,
  and the script correctly refused. Checksum and attestation checks are unchanged.
