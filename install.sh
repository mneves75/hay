...
#
#   curl -fsSL https://raw.githubusercontent.com/mneves75/hay/main/install.sh | sh
#   …or inspect first:
...
# MSRV gate: hay is edition 2024 / rustc >= 1.85.
...
if [ "$(rustc_major)" -lt 1 ] || { [ "$(rustc_major)" -eq 1 ] && [ "$(rustc_minor)" -lt 85 ]; }; then
  echo "hay installer: rustc >= 1.85 required, found $(rustc --version)" >&2
  exit 2
fi
...
echo "==> installing to $(dirname "$(command -v cargo)")/bin"
...
if command -v hay >/dev/null 2>&1; then
  ...
  echo "    Add \"$(dirname "$(command -v cargo)")/bin\" to your PATH." >&2
fi
